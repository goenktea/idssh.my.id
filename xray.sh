...
getData() {
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        if [[ -f ~/xray.crt && -f ~/xray.key ]]; then
            ...
            CERT_FILE="/usr/local/etc/xray/v2ray.crt"
            KEY_FILE="/usr/local/etc/xray/v2ray.key"
        else
    ...
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        if [[ -z "$answer" ]]; then
            ...
            2)
                ...
                ;;
            3)
                PROXY_URL="https://tinyurl.com/idssh"
                ;;
            4)
                PROXY_URL="https://bing.imeizi.me"
                ;;
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        cp ~/xray.crt /usr/local/etc/xray/v2ray.crt
        cp ~/xray.key /usr/local/etc/xray/v2ray.key
    fi
}
...
EOF
    ...
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        # VLESS+WS+TLS
        if [[ "$WS" = "true" ]]; then
            cat > ${NGINX_CONF_PATH}v2ray.conf<<-EOF
...
uninstall() {
    ...
    if [[ "${answer,,}" = "y" ]]; then
        ...
        if [[ "$domain" != "" ]]; then
            rm -rf ${NGINX_CONF_PATH}v2ray.conf
        fi
...
getConfigFileInfo() {
    ...
    if [[ "$ws" = "true" ]]; then
        port=`grep -i ssl $NGINX_CONF_PATHv2ray.conf| head -n1 | awk '{print $2}'`
    else
